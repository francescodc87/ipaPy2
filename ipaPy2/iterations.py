        ...
        if isoid is not None:
            ...
            for isp in range(0,len(hits)):
                ...
                if(len(ISt.index)<len(ISm.index)):
                    ...
                    if ch!=Charge:
                        pMs= [(stats.norm(0, ppm).pdf(2*ppm)/stats.norm(0, ppm).pdf(0))/1000]*len(ISm.index)
                        pIs= [(stats.lognorm(scale=ISt.iloc[0,1],s=sigmaln).pdf(ISt.iloc[0,1]+ratiounk*ISt.iloc[0,1])/stats.lognorm(scale=ISt.iloc[0,1],s=sigmaln).pdf(math.exp(ISt.iloc[0,1]-(sigmaln)**2)))/1000]*len(ISm.index)
